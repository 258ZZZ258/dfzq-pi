...
Server 使用 `PIPELINE_DB_DSN` 指向 PostgreSQL；首次部署前，在专用空数据库或 schema
执行 `migrations/001_task_runs.postgresql.sql`，创建运行记录和事件表。启动服务不会自动建表。
若已有同名表，先核对 `src/store/postgres.ts` 的字段与约束，不重复执行初始迁移，也不删除现有记录。
当前启动恢复会将该库中排队和运行中的任务标为中断，因此不同环境必须隔离，不能共用运行历史库启动第二个实例。
