 ...
 *    第 1 点的 `..` 折叠在没有 `..` 时是恒等变换,所以 guard 与 pi 在"是否存在"这一步看到
 *    的其实是同一个判断:`resolved` 存在 ⇔ `raw` 存在,回退分支的触发条件("resolved 不
 *    存在")对任何被 guard 放行的输入恒为假——回退代码对 guard 放行的路径是死代码。
 ...
 *
 * 这份清单只覆盖"已经被复审用真实向量证明过分歧"的两类(第 1、2 点),和两类记录在案但
 * 未独立关闭的已知缺口(第 3、4 点)。**消费方新增一种字符串变换,这里不会自动感知**——
