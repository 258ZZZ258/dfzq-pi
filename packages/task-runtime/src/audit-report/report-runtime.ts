...
export async function createAuditReportRuntime(
	...
	const { runTimeoutMs, maxTotalTokens, maxCostUsd } = options.spec.limits;
	if (!runTimeoutMs || !maxTotalTokens || !maxCostUsd)
		throw new Error("Audit narrative runtime requires explicit time, token and cost limits");
	let active:
		| {
	...
	return {
		...
		async run(input, runOptions) {
			...
			try {
				...
				// Completion costs are measured after each response; combined overspend must not be delivered as success.
				const limit = timedOut
					? "runTimeout"
					: result.usage.total > maxTotalTokens
						? "maxTotalTokens"
						: result.usage.cost > maxCostUsd
							? "maxCostUsd"
							: result.limit;
				if (limit) {
