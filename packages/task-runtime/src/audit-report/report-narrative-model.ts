...
export function createNarrativeModelCaller(options: NarrativeModelOptions): {
	...
	if (
		...
		!Number.isFinite(options.maxTotalTokens) ||
		options.maxTotalTokens <= 0 ||
		!Number.isFinite(options.maxCostUsd) ||
		options.maxCostUsd <= 0
	)
		throw new Error("Narrative model calls require a finite deadline and positive budgets");
	...
	return {
		...
		callModel: async (request, signal) => {
			...
			if (busy) throw new Error("Concurrent narrative calls would race the shared budget");
			busy = true;
			try {
				const remainingMs = options.deadlineAt - Date.now();
				const remainingTokens = Math.floor(options.maxTotalTokens - usage.total);
				const remainingCost = options.maxCostUsd - usage.cost;
				if (remainingMs <= 0 || remainingTokens <= 0 || remainingCost <= 0)
					throw new Error("Narrative processing budget exhausted");
				...
				const spec: RuntimeSpec = {
					...
					limits: {
						...
						maxTotalTokens: Math.min(original.limits.maxTotalTokens ?? remainingTokens, remainingTokens),
						maxCostUsd: Math.min(original.limits.maxCostUsd ?? remainingCost, remainingCost),
					},
				};
