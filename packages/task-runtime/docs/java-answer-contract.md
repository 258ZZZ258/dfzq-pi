...
路径产出不达标(契约校验不过、反幻觉校验不过、检索没拿到可用结果,或阶段 1 本身超时/撞限额/抛错)时,自
动升级到既有的 agent 自主编排路径重新跑一遍。**升级过程对 Java 完全透明** —— 从 HTTP 契约的角度看不出区
别,一个 `runId`、一个终态,响应体形状与 §2 完全一致;唯一不同的是下表几个字段的取值口径。
...
| `durationMs` | 快路径这一次的耗时 | **两阶段之和** |

⇒ **不要用 `turns` 反推模型调用了几次** —— 升级过的 run 里它是两段相加,不是单一阶段的轮次。
